...
export const AdminPortal: React.FC<AdminPortalProps> = ({
  ...
  // Active Admin Tab
  const [activeTab, setActiveTab] = useState<'stocks' | 'flacons' | 'orders' | 'customers' | 'ads' | 'analytics' | 'users'>('stocks');
  ...
  const [stockFamilyFilter, setStockFamilyFilter] = useState<'all' | 'Extrait' | 'Flacon'>('all');
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      // Preserve all existing manually added Flacons
      const existingFlacons = products.filter((p) => p.family === 'Flacon');
      const updated = [...existingFlacons, ...newExtraits];

      onUpdateProducts(updated);

      setExcelSuccess(
        `${newExtraits.length} Extraits importés avec succès. L'ancien inventaire d'extraits a été remplacé, et vos ${existingFlacons.length} modèles de flacons restent conservés.`
      );
    } catch (err: any) {
          ...
          if (prodSheetName) {
            ...
            if (rawProd.length > 0) {
              foundProducts = rawProd.map((r, i) => ({
                ...
                family: (r.Famille === 'Flacon' ? 'Flacon' : 'Extrait') as Product['family'],
                category: 'Standard',
                unit: r.Famille === 'Flacon' ? '1 pièce' : '1g (Contenant 100g)',
