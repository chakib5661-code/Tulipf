...
export default function App() {
  ...
  // 7. Multi-Language State (French, Arabic, English)
  ...
  // Ask customer to select language on first visit and save so we never ask again
  useEffect(() => {
    try {
      const hasChosenLang = localStorage.getItem('tulip_lang_chosen_v1');
      if (!hasChosenLang) {
  ...
  // Advertising popup on initial visit (shows random active banner after main page is loaded)
  useEffect(() => {
    ...
    const activeAds = adBanners.filter((b) => b.isActive);
  ...
  }, [isMainPageLoaded, currentView, isAdPopupEnabled, adBanners]);
